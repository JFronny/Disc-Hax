...
namespace Bot
{
    public static class CommandArr
    {
        ...
        private static IEnumerable<Tuple<Type, string>> GetGroups() => _groups ??= Assembly.GetExecutingAssembly().GetTypes().Where(s => typeof(BaseCommandModule).IsAssignableFrom(s)).Select(s => new Tuple<Type, string>(s,
        ...
        public static IEnumerable<string> GetCommandNames(string group) => GetCommandNames(_groups.First(s => string.Equals(s.Item2, group, StringComparison.CurrentCultureIgnoreCase)).Item1);
        ...
        public static string[] GetCommandNames() => (_commandNames ??= GetGroups().Select(s => s.Item1).SelectMany(GetCommandNames)).ToArray();
    }
}