...
namespace Bot
{
    public class Commands
    {
        ...
        {
            if (Config.data.Chan)
                await Chan(args, ctx.Channel, (c1, c2, c3) => ctx.RespondAsync(c1, c2, c3));
        }
        ...
        public static async Task Chan(string[] args, DiscordChannel Channel, Func<string, bool, DiscordEmbed, Task<DiscordMessage>> postMessage)
        {
            if (args.Length == 0)
            {
                ...
                await postMessage.Invoke(string.Join("\r\n", lmsg) + "\r\nUsage: !4chan <ShortName>", false, null);
            }
            else
            {
                ...
                if (Channel.IsNSFW || Config.data.Nsfw)
                {
                    ...
                    await postMessage.Invoke(null, false, new DiscordEmbedBuilder { Title = t.Name + "#" + t.Id + ": " + (string.IsNullOrWhiteSpace(t.Subject) ? "Untitled" : t.Subject), ImageUrl = t.Image.Image.AbsoluteUri });
                }
                else
        ...
        public async Task Waifu(CommandContext ctx, [Description("Use \"f\" to force execution, even on non-NSFW channels")] params string[] args)
        {
            if (Config.data.Waifu)
                await Waifu(args, ctx.Channel, (c1, c2, c3) => ctx.RespondAsync(c1, c2, c3));
        }
        ...
        {
            if (Config.data.Play)
                await Play((c1, c2, c3) => ctx.RespondAsync(c1, c2, c3));
        }

        public static async Task Play(Func<string, bool, DiscordEmbed, Task<DiscordMessage>> postMessage) => await postMessage.Invoke("No.", false, null);
        ...
        public async Task Booru(CommandContext ctx, [Description("Tags for image selection")] params string[] args)
        {
            if (Config.data.Booru)
                await Booru(args, ctx.Channel, (c1, c2, c3) => ctx.RespondAsync(c1, c2, c3));
        }
        ...
        public async Task ConfigCmd(CommandContext ctx)
        {
            if (Config.data.Config)
                await ctx.RespondAsync(Config.data.ToString(true));
        }

        public static async Task ConfigCmd(Func<string, bool, DiscordEmbed, Task<DiscordMessage>> postMessage) => await postMessage.Invoke(Config.data.ToString(true), false, null);
    }
}
