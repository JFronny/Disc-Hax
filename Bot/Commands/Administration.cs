    ...
    public class Administration : BaseCommandModule
    {
            ...
            params string[] args)
        {
            if (ConfigManager.get(ctx.Channel.Id, ConfigElement.Enabled)
                .AND(ConfigManager.get(ctx.Channel.Id, ConfigElement.Config)))
            {
                if (args.Length == 0)
                {
                    await ctx.RespondAsync(ConfigManager.getStr(ctx.Channel.Id));
                }
                else
                {
                    ...
                    if (args.Length == 1)
                    {
                        await ctx.RespondAsync(el + ": " + ConfigManager.get(ctx.Channel.Id, el));
                    }
                    else
                    {
